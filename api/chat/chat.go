...
func ClientGenAI() *genai.Client {
	...
	client, err := genai.NewClient(ctx, option.WithAPIKey(utils.DapatinEnvVariable("GEMINI_AI")))
	if err != nil {
		log.Fatal(err)
	}
...
func BuatChat(client *genai.Client, karakter models.Karakter, personalitas models.Personalitas, history []*genai.Content) *genai.ChatSession {
	model := BuatModel(client, &genai.Content{
		Parts: []genai.Part{
			genai.Text(karakter.RenderPersonalitas(personalitas.Nama) + "\n" + "To show an expression, please put '*' between the expression sentence like, '*Feeling surprised*'" + "\n" + personalitas.Personalitas),
...
func SaranKalimat(client *genai.Client, personalitas models.Personalitas, cs *genai.ChatSession) *genai.GenerateContentResponse {
